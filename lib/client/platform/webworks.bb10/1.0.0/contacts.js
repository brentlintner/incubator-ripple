...
function get() {
    ...
    function intoContactObj(properties) {
        ...
        utils.forEach(properties, function (value, prop) {
            contact[prop] = value;
        });
    ...
    return (contactsDB || defaultContacts).map(intoContactObj);
}
...
(function () {
    event.on("webworks-bb10-contact-contact-save", function (contactProperties, success) {
        var contacts = get(),
    ...
    event.on("webworks-bb10-contact-remove", function (id, success, error) {
        if (!id) {
            // TODO: Correct error?
            error(error, ContactError.PENDING_OPERATION_ERROR, "id is falsy (" + id + ")");
        } else {
            var contacts = get(),
